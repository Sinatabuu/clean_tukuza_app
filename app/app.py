...
# ... (previous code) ...
    ...
    # Always display previous results and the clear button if they exist
    if "gift_results" in st.session_state.user_profile:
        ...
        if st.button("🧹 Clear Previous Gift Assessment", key="clear_gift_assessment_button"):
            ...
            st.rerun() # Rerun to re-display the form
    ...
    sample_input = st.text_input("🌐 Type anything in your language to personalize the experience:", key="sample_lang_input_assessment") # Changed key for uniqueness
    ...
    # The form (with sliders and submit button) is only defined here
    # if no results are present from a previous assessment.
    with st.form("gift_assessment_form", clear_on_submit=True):
        ...
        if submitted:
            try:
                ...
                # Trigger a rerun to display the results and hide the form
                st.rerun()
